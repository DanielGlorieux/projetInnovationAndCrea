...
Fonctionnalités :
    ...
    - Hover sur les points pour afficher les identifiants des vecteurs
...
class IVFSimulator:
    ...
    def _setup_figure(self):
        ...
        self.fig.text(
            0.32,
            0.935,
            "Cliquez dans la zone pour placer un vecteur requête | Survolez un point pour voir son identifiant",
            ha="center",
            va="center",
            fontsize=10,
            color="#aaaaaa",
        ...
        # Annotation pour le hover (initialement invisible)
        self._hover_annot = self.ax_main.annotate(
            "",
            xy=(0, 0),
            xytext=(12, 12),
            textcoords="offset points",
            bbox=dict(boxstyle="round,pad=0.4", fc="#1a1a2e", ec="#e94560", lw=1.5),
            fontsize=8,
            color="white",
            family="monospace",
            zorder=10,
        ...
        # Connecter le mouvement de la souris
    ...
    def _draw_main(self):
        ...
        # Recréer l'annotation de hover après ax.clear()
    ...
    def _on_hover(self, event):
        """Affiche l'identifiant du vecteur au survol de la souris."""
        if event.inaxes != self.ax_main or self.vectors is None:
            if self._hover_annot.get_visible():
                self._hover_annot.set_visible(False)
                self.fig.canvas.draw_idle()
            return

        # Calculer la distance du curseur à chaque vecteur (en coordonnées data)
        cursor = np.array([event.xdata, event.ydata])
        dists = np.sum((self.vectors - cursor) ** 2, axis=1)
        nearest_idx = int(np.argmin(dists))

        # Seuil de distance en coordonnées data pour afficher le tooltip
        # On utilise un seuil adapté à l'étendue du graphe (0 à 10)
        threshold = 0.15
        if dists[nearest_idx] > threshold**2:
            if self._hover_annot.get_visible():
                self._hover_annot.set_visible(False)
                self.fig.canvas.draw_idle()
            return
        ...
        vec = self.vectors[nearest_idx]
        cluster_id = self.labels[nearest_idx]
        self._hover_annot.xy = (vec[0], vec[1])
...
def main():
    ...
    print("  - Survolez un point pour afficher son identifiant")
